...
<!DOCTYPE html>
    <html>
        <head>
            <meta charset="utf-8"/>
            <title>Slim Framework grupi.org</title>
            <style>
        ...
        <body>
            ...
            <h1>Welcome to <a href="http://api.grupi.org">api.grupi.org<a/>  </h1>
            <p>
                api.grupi.org use Slim framework, you must have token id to use this api.</a>.
            </p>
            <section>
                <h2>Get Started</h2>
                <ol>
                    <li>Open the token id to <code>api.grupi.org/token_id</code></li>
                    <li>Fetch json web service with any program, example : android apps</li>
                    <li>Enjoy to use grupi</li>
                </ol>
            </section>
            <section>
...
EOT;
  ...
  $app->get('/:name', function ($name) {
    $sql = "SELECT * FROM rekomendasi.$name";
    try {
        $db = getConnection();
      //mysql_select_db('api');
        $stmt = $db->query($sql);
       ...
       echo '
          ...
          <body><h1>404 Page Not Found</h1><p>The page you are looking for could not be found. Check the address bar to ensure your URL is spelled correctly. If all else fails, you can visit our home page at the link below.</p><a href="/">Visit the Home Page</a></body>
          </html>
         ';
          }
  });
  
    $app->post('/nilai', function () use($app) {
    global $app;
    $req = $app->request();
    $body = json_decode($req->getBody());
    $sql = "INSERT INTO rekomendasi.nilai_rekomendasi (id_mahasiswa, nama_matkul_r) VALUES ( :id_mahasiswa , :nama_matkul_r );";
  try {
    $db = getConnection();
    $stmt = $db->prepare($sql);
      $stmt->bindParam("id_mahasiswa", $body->id_mahasiswa);
      $stmt->bindParam("nama_matkul_r", $body->nama_matkul_r);
      $stmt->execute();
      $db = null;
      echo "suskes bero";
  } catch(PDOException $e) {
